...
# ── Drop Hold (0) rows — binary Buy vs Sell ──────────────────────────────────
...
forest = RandomForestFromScratch(
    n_estimators=200,
    max_depth=10,
...
RandomForestFromScratch.evaluate(Y_train, proba_train)

print("\n--- Test Set ---")
proba_test = forest.predict_proba(X_test)
RandomForestFromScratch.evaluate(Y_test, proba_test)
...
importance_pairs = sorted(
    zip(feature_cols_z, forest.feature_importances_),
    key=lambda x: x[1],
    reverse=True,
)
